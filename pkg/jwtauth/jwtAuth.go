...
func NewAccessToken(secret string, expiresAt int64, claims *Claims) AuthFactory {
	return &accessToken{
		authConcrete: &authConcrete{
			...
			Claims: &AuthMapClaims{
				...
				RegisteredClaims: jwt.RegisteredClaims{
					...
					ExpiresAt: JwtTimeDurationCal(expiresAt),
...
func ParseToken(secret string, tokenString string) (*AuthMapClaims, error) {
	...
	if err != nil {
		if errors.Is(err, jwt.ErrTokenMalformed) {
			return nil, errors.New("error: token malformed")
