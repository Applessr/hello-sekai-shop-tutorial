...
func (r *playerRepository) playerDbConnect(pctx context.Context) *mongo.Database {
...
func (r *playerRepository) InsertOnePlayer(pctx context.Context, req *player.Player) (primitive.ObjectID, error) {
	...
	playerId, err := col.InsertOne(ctx, req, nil)
	if err != nil {
		log.Printf("Error: InsertOnePlayer: %s", err.Error())
		return primitive.ObjectID{}, errors.New("error: Insert one player failed")
	}
	return playerId.InsertedID.(primitive.ObjectID), nil
}

func (r *playerRepository) FindOnePlayerProfile(pctx context.Context, id string) (*player.PlayerProfileBson, error) {
	ctx, cancel := context.WithTimeout(pctx, 10*time.Second)
	defer cancel()
	...
	if err := col.FindOne(
		ctx,
		bson.M{"_id": utils.ConvertToObjectId(id)},
...
func (r *playerRepository) FindOnePlayerProfileToRefresh(pctx context.Context, playerId string) (*player.Player, error) {
	...
	if err := col.FindOne(ctx, bson.M{"_id": utils.ConvertToObjectId(playerId)}).Decode(result); err != nil && err != mongo.ErrNoDocuments {
